...
1. Add a component that inherits from **Renderer**. **Mesh Renderer** is the **most common** and is **suitable for most use cases**, but **`SkinnedMeshRenderer`**, **`LineRenderer`**, **`BillboardRenderer`**, or **`TrailRenderer`** might be more suitable if your GameObject has **special requirements**.
